...
mod private_staging_tests {
    ...
    fn private_staging_is_owner_only_even_under_a_permissive_umask() -> TestResult {
        ...
        std::fs::set_permissions(&parent, std::fs::Permissions::from_mode(0o755))
            .map_err(|error| error.to_string())?;
        let first =
            create_publish_staging_dir(&parent.join("index")).map_err(|error| error.to_string())?;
        let second =
            create_publish_staging_dir(&parent.join("index")).map_err(|error| error.to_string())?;
    ...
    fn private_staging_survives_activation_retention_and_failed_commit() -> TestResult {
        ...
        let error = publish_staged_index_with_commit(&index, &second, || {
            ...
            Err(IndexRebuildError::Index(
                "controlled job commit failure".to_owned(),
            ))
        });
        assert!(error.is_err());
        assert_eq!(
            std::fs::read(index.join("body")).map_err(|error| error.to_string())?,
            b"first"
        );
        private(&index)?;
        ...
        for entry in std::fs::read_dir(&parent).map_err(|error| error.to_string())? {
            ...
            private(&entry.path())?;
            contents.insert(
                std::fs::read(entry.path().join("body")).map_err(|error| error.to_string())?,
            );
        }
        assert_eq!(
            contents,
            BTreeSet::from([b"first".to_vec(), b"rejected".to_vec(), b"current".to_vec()])
        );
        Ok(())
    }
    ...
    fn private_staging_does_not_follow_or_modify_an_untrusted_parent() -> TestResult {
        ...
        assert_eq!(
            std::fs::read(real.join("existing")).map_err(|error| error.to_string())?,
            b"unchanged"
        );
        assert_eq!(
...
fn sync_index_generation_with(
    ...
    while let Some((path, visited)) = pending.pop() {
        ...
        if metadata.file_type().is_dir() {
            ...
            return Err(IndexRebuildError::Index(
                "Refusing to flush an index generation containing a symlink or special entry"
                    .to_owned(),
            ));
        }
    }
...
fn sync_index_entry(path: &Path, directory: bool) -> Result<(), IndexRebuildError> {
    ...
    {
        return Err(IndexRebuildError::Index(
            "Index durability entry changed type".to_owned(),
        ));
    }
    file.sync_all().map_err(|error| {
        IndexRebuildError::Index(format!(
            "Failed to flush index data before publication: {error}"
        ))
    })
}
    ...
    F: FnOnce() -> Result<(), IndexRebuildError>,
{
    ...
    let commit_result = sync_directory(index_parent(index_dir)).and_then(|()| {
        ...
        if self.armed {
            ...
            if let Some(retained) = self.retained_dir {
                if path_exists_no_follow(self.index_dir) {
                    return Err(IndexRebuildError::Index(
                        "Refusing to overwrite an unexpected active index during restoration"
                            .to_owned(),
                    ));
                }
                rename_index_dir(
...
fn publish_index_by_exchange(
    ...
    let mut rollback = IndexExchangeRestoreGuard {
        index_dir,
        displaced: staging_dir,
        armed: true,
    };
...
fn rollback_published_index_namespace(
    ...
    {
        return rollback_index_by_exchange_with(
            index_dir,
            retained,
            |from, to| {
                rename_index_dir(from, to, "quarantine atomically rejected index generation")
            },
...
/// Flush those mutations even when rollback is already going to report an
/// error. This does not claim power-loss durability when the flush fails.
fn finish_index_rollback(
    index_dir: &Path,
    mut rollback_errors: Vec<String>,
    mut sync_directory: impl FnMut(&Path) -> Result<(), IndexRebuildError>,
) -> Result<(), IndexRebuildError> {
    if let Err(error) = sync_directory(index_parent(index_dir)) {
        rollback_errors.push(format!("failed to persist index rollback: {error}"));
    }
...
mod tests {
    ...
    fn publication_durability_flushes_every_payload_before_its_directory() -> TestResult {
        ...
        sync_index_generation_with(
            &path,
            || Ok(()),
            |entry, directory| {
                sync_index_entry(entry, directory)?;
                flushed.push((
                    entry.strip_prefix(&path).unwrap_or(entry).to_path_buf(),
                    directory,
                ));
                Ok(())
            },
        ...
        let expected = vec![
            ...
            flushed == expected,
            format!("unexpected durability order: {flushed:?}"),
        )?;
        ensure(
            index_regular_file_snapshot(&path)? == before,
            "sync must not change payload bytes",
        )
    }
    ...
    fn publication_durability_stops_on_payload_error_or_cancellation() -> TestResult {
        ...
        let result = sync_index_generation_with(
            &path,
            || Ok(()),
            |entry, directory| {
                if entry.file_name().is_some_and(|name| name == "b.quality") {
                    return Err(IndexRebuildError::Index(
                        "injected payload sync failure".to_owned(),
                    ));
                }
                sync_index_entry(entry, directory)?;
            ...
            result.is_err() && seen == [path.join("a.fast")],
            "must stop before marking tree durable",
        )?;
        let mut checks = 0;
        let mut syncs = 0;
        let result = sync_index_generation_with(
            ...
            || {
                ...
                if checks == 2 {
                    Err(IndexRebuildError::Index(
                        "injected checkpoint cancellation".to_owned(),
                    ))
                } else {
                    Ok(())
                }
            ...
            result.is_err() && checks == 2 && syncs == 0,
            "payload flushing must remain cancellable",
        )?;
        ensure(
            index_regular_file_snapshot(&path)? == before,
            "failed sync must preserve payload bytes",
        )
    }
    ...
    fn publication_durability_refuses_nested_symlinks_and_special_entries() -> TestResult {
        ...
        for tree in [&linked, &socket_dir] {
                ...
                "non-regular entries must not be followed",
            )?;
        }
        ensure(
            read_marker(&outside, "private")? == "private source remains unchanged",
            "outside data must not change",
        )
    }
    ...
    fn publication_durability_orders_directory_barriers_before_commit() -> TestResult {
        ...
        publish_staged_index_with_sync(
            &active,
            &staged,
            || {
                let calls = checkpoints.borrow();
                if calls.len() != 4
                    || read_marker(&active, "generation").ok().as_deref() != Some("new")
                {
                    return Err(IndexRebuildError::Index(
                        "commit ran before all directory barriers".to_owned(),
                    ));
                }
                Ok(())
            },
            |directory| {
                ...
                checkpoints.borrow_mut().push((
                    directory.to_path_buf(),
                    active.exists(),
                    staged.exists(),
                    path.join("index.previous").exists(),
                ));
                Ok(())
            },
        )
        .map_err(|error| error.to_string())?;
        let retained_barrier = if cfg!(any(
            target_os = "linux",
            target_os = "android",
            target_vendor = "apple"
        )) {
    ...
    fn publication_durability_failure_at_each_barrier_preserves_previous_generation() -> TestResult
    {
        for fail_at in 1..=4 {
            ...
            let mut count = 0;
            let mut committed = false;
            let result = publish_staged_index_with_sync(
                &active,
                &staged,
                || {
                    committed = true;
                ...
                result.is_err() && !committed,
                format!("barrier {fail_at} must withhold commit"),
            )?;
            ensure(
                index_regular_file_snapshot(&active)? == before,
                format!("barrier {fail_at} lost old generation"),
            )?;
            if fail_at == 4 {
                let rejected = rejected_generation_dirs(&path, "index")?;
                ensure(
                    rejected.len() == 1 && read_marker(&rejected[0], "generation")? == "new",
                    "activation sync failure must quarantine new generation",
                )?;
            } else {
                ensure(
                    read_marker(&staged, "generation")? == "new",
                    "pre-activation failure must retain staged bytes",
                )?;
            }
        }
        Ok(())
    ...
    fn publication_durability_new_store_failure_quarantines_uncommitted_generation() -> TestResult {
        ...
        let result = publish_staged_index_with_sync(
            ...
            |directory| {
                ...
                if count == 3 {
                    Err(IndexRebuildError::Index(
                        "injected first activation sync failure".to_owned(),
                    ))
                } else {
                    sync_index_directory(directory)
                }
            ...
            result.is_err() && !committed && !active.exists(),
            "failed first publish must remain absent",
        )?;
        let rejected = rejected_generation_dirs(&path, "index")?;
        ensure(
            rejected.len() == 1 && read_marker(&rejected[0], "generation")? == "new",
    ...
    fn publication_durability_unwind_at_retention_or_activation_restores_old_bytes() -> TestResult {
        for fail_at in [3, 4] {
            ...
            let mut count = 0;
            let unwind = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                let _ = publish_staged_index_with_sync(
                    &active,
                    &staged,
                    || Ok(()),
                    |directory| {
                ...
                unwind.is_err(),
                "injected panic must escape after restoring the generation",
            )?;
            ensure(
                index_regular_file_snapshot(&active)? == before,
                "unwind must preserve active bytes",
            )?;
            ensure(
                !path.join("index.previous").exists(),
                "unwind must restore retained name",
            )?;
        }
        Ok(())
    }
    ...
    fn publication_exchange_keeps_live_name_present_at_each_directory_barrier() -> TestResult {
        ...
        .map_err(|error| error.to_string())?;
            ...
            read_marker(&path.join("index.previous"), "generation")? == "old",
        ...
        for panic_after_swap in [false, true] {
            ...
            let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                publish_index_by_exchange(&active, &staged, &mut sync_index_directory, |from, _| {
                    ...
                    {
                        return Err(IndexRebuildError::Index(
                            "kernel exchange did not take place".to_owned(),
                        ));
                    }
                    attempted.set(true);
                    if panic_after_swap {
                    ...
                    Err(IndexRebuildError::Index(
                        "injected retention failure after atomic exchange".to_owned(),
                    ))
                })
            }));
            ensure(
                attempted.get(),
                "test must reach the post-exchange boundary",
            )?;
            ensure(
                if panic_after_swap {
                    result.is_err()
                ...
                index_regular_file_snapshot(&active)? == before,
                "atomic rollback must restore old bytes",
            )?;
            ensure(
                read_marker(&staged, "generation")? == "new",
                "failed candidate must remain inspectable",
            )?;
        }
        Ok(())
    }
    ...
    #[cfg(any(target_os = "linux", target_os = "android", target_vendor = "apple"))]
    #[test]
    fn publication_exchange_readers_never_observe_a_missing_generation_during_replacement()
    -> TestResult {
        let root = tempfile::tempdir().map_err(|error| error.to_string())?;
        let path = root
            .path()
        ...
        std::thread::scope(|scope| -> TestResult {
                ...
                for _ in 0..256 {
                    ...
                    let result = std::fs::read(active.join("generation"))
                        ...
                        .and_then(|body| {
                                ...
                                body == b"old" || body == b"new",
                                "reader observed a mixed payload",
                            )
                        });
                    if failure.is_none() {
                        failure = result.err();
            ...
            for _ in 0..256 {
                ready.wait();
                let result = exchange_index_directories(&active, &candidate)
                    .map_err(|error| error.to_string());
                if failure.is_none() {
                    failure = result.err();
                }
    ...
    fn publication_exchange_directory_rename_refuses_even_an_empty_existing_target() -> TestResult {
        ...
        write_marker(
            &source,
            "generation",
            "do not move into an existing destination",
        )?;
        std::fs::create_dir(&target).map_err(|error| error.to_string())?;
        ensure(
            rename_index_dir(&source, &target, "test no-clobber retention").is_err(),
            "an existing empty directory must not be silently overwritten",
        )?;
        ensure(
            source.is_dir() && target.is_dir() && !target.join("generation").exists(),
